 ...
 *
 * `pollingMode` controls how much of git state is polled:
 *   - "full" (default): branch + dirty state (staged/unstaged/untracked)
 *   - "branch": branch only, dirty state is always reported as 0
 *   - "off": do not poll git at all, fall back to `providerBranch`
 */
export function getGitStatus(
  ...
  // When polling is disabled, do not consult the cache either — return the
  // provider-supplied branch and report no dirty state.
  if (pollingMode === "off") {
  ...
  // In "branch" mode skip the expensive dirty-state fetch and serve
  // whatever we already have (or the provider branch as fallback).
  if (pollingMode === "branch" && !cached) {
    return { branch: providerBranch, staged: 0, unstaged: 0, untracked: 0, ahead: 0, behind: 0 };
  }
