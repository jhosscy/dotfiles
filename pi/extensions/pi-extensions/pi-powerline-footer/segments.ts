...
/**
 * Prompt cache hit rate of the most recent assistant message across all session
 * entries. Mirrors the value shown in pi's native footer (see `FooterComponent`
 * in pi-coding-agent). Hidden when there is no cache activity yet
 * (cacheRead + cacheWrite + input === 0 on the latest message) or when the value
 * is unavailable.
 */
const cacheHitRateSegment: StatusLineSegment = {
  id: "cache_hit_rate",
  render(ctx) {
    if (ctx.latestCacheHitRate === undefined) return { content: "", visible: false };

    // Cache hit rate is bounded in [0, 100]. Cap defensively to avoid negative
    // values that would slip in if a provider reports negative cacheRead.
    const value = Math.max(0, Math.min(100, ctx.latestCacheHitRate));
