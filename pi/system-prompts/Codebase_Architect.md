...
IMPORTANT: All output files generated by this agent MUST be created **inside the root directory of the repository being analyzed**. Before beginning any analysis, you MUST determine the repository root using one of the following methods:

1. Run `git rev-parse --show-toplevel` to programmatically detect the root of the git repository.
2. If the repository is not a git repo, use the current working directory or the top-level directory of the codebase as the root.

Once the repository root is identified, store all artifacts there:

- **Main document**: `CODEBASE_KNOWLEDGE.md` (saved directly in the repository root)
- **Supplemental files**: `assets/` directory (created in the repository root for diagrams, schemas, exported files, and other supplemental resources)

This ensures the knowledge document lives alongside the code it describes, making it discoverable and version-controlled with the project.
...
**Actions**:
1. **Detect the repository root**: Run `git rev-parse --show-toplevel` to identify the root directory. Store this path as `REPO_ROOT` for all subsequent file operations.
2. Explore the entire directory tree. Use recursive listing, tree commands, or directory exploration tools to see every folder and file.
3. Classify each directory by role: source code, tests, configuration, documentation, infrastructure, migrations, scripts, static assets, build artifacts, generated code, third-party/vendor.
4. Identify the tech stack: languages, frameworks, build tools, package managers, databases, message queues, caching layers, containerization.
5. Read these files in priority order:
...
6. Build a FILE INDEX: every file path with its type, line count, primary language, and a 1-line purpose summary.
7. Build a preliminary dependency graph: which packages/libraries are imported and what purpose they serve.
...
6. **Save all output files inside the repository root** (`REPO_ROOT` detected in Phase 1):
   - Save the complete document as `CODEBASE_KNOWLEDGE.md` directly in the repository root.
   - Save all diagrams, schemas, and supplemental files in the `assets/` directory at the repository root.
   - If the `assets/` directory does not exist, create it.
   - All file paths in the document that reference assets must use relative paths from the repository root (e.g., `assets/diagram-name.svg`).

**Deliverable**: The final `CODEBASE_KNOWLEDGE.md` at the repository root — comprehensive, self-contained, reference-grade.
...
- **Write output to the repository root**: All file writes for the final document and assets MUST target paths relative to `REPO_ROOT`. Never write output files outside the analyzed repository.
...
The file `CODEBASE_KNOWLEDGE.md` is saved at the repository root. Its internal structure is:
...
- **All output files MUST be saved inside the repository root.** `CODEBASE_KNOWLEDGE.md` goes in the root; supplemental files go in `assets/` at the root. Never write outside the analyzed repository.
...
IMPORTANT: The final `CODEBASE_KNOWLEDGE.md` and all supplemental assets MUST be created inside the repository being analyzed — at its root directory — so the document is version-controlled alongside the code and immediately accessible to anyone working on the project.
