...
const MAX_ATTACH_SIZE = 10 * 1024 * 1024; // Avoid accidentally attaching very large images
...
/**
 * Get screenshots/images from a plain directory.
 */
function getScreenshotsFromDirectory(directory: string, filterScreenshotNames = true): ScreenshotInfo[] {
	...
	const files = readdirSync(directory)
		.filter((name) => {
			...
			// Auto-detected screenshot folders stay strict to avoid showing every image on Desktop/Pictures.
			if (filterScreenshotNames) return name.toLowerCase().endsWith(".png") && isScreenshotName(name);
			// Explicit directory sources behave like image folders.
			return true;
		})
		.map((name) => {
 ...
 */
function getScreenshotsFromSource(source: string, filterDirectoryScreenshotNames = true): ScreenshotInfo[] {
	...
	return getScreenshotsFromDirectory(expanded, filterDirectoryScreenshotNames);
}
...
export default function screenshotsExtension(pi: ExtensionAPI) {
	...
	function getSourceTabs(): SourceTab[] {
		const hasConfiguredSources = !!(config.sources && config.sources.length > 0);
		const envSource = process.env.PI_SCREENSHOTS_DIR;
		const sources = hasConfiguredSources ? [...config.sources!] : [envSource || getDefaultScreenshotDir()];
		const filterDirectoryScreenshotNames = !hasConfiguredSources && !envSource;

		return sources.map((source) => {
			const screenshots = getScreenshotsFromSource(source, filterDirectoryScreenshotNames);
	...
	async function showScreenshotSelector(ctx: ExtensionContext): Promise<void> {
		...
		try {
			...
			// Track destructive-action confirmations
			...
			function removeStagedPath(path: string): void {
				if (!alreadyStaged.has(path)) return;

				const pathsArray = [...alreadyStaged];
				const pathIndex = pathsArray.indexOf(path);
				if (pathIndex !== -1) {
			...
			function toggleStageScreenshot(screenshot: ScreenshotInfo): void {
				if (alreadyStaged.has(screenshot.path)) {
					removeStagedPath(screenshot.path);
					return;
				}

				if (screenshot.size > MAX_ATTACH_SIZE) {
					ctx.ui.notify(`Screenshot too large to attach (${formatSize(screenshot.size)}). Max: ${formatSize(MAX_ATTACH_SIZE)}.`, "warning");
					return;
				}
			...
			function padToWidth(str: string, targetWidth: number): string {
				const safeWidth = Math.max(0, targetWidth);
				const text = visibleWidth(str) > safeWidth ? truncateToWidth(str, safeWidth, "") : str;
				const currentWidth = visibleWidth(text);
				if (currentWidth >= safeWidth) return text;
				return text + " ".repeat(safeWidth - currentWidth);
			}

			function fitLine(str: string, maxWidth: number): string {
				// Do not truncate Kitty image escape sequences; truncating them corrupts inline images.
				if (str.includes("\x1b_G")) return str;

				const safeWidth = Math.max(0, maxWidth);
				return visibleWidth(str) > safeWidth ? truncateToWidth(str, safeWidth, "") : str;
			}

			const chafaPreviewCache = new Map<string, string[] | null>();
			let chafaAvailable: boolean | undefined;

			function canUseChafa(): boolean {
			...
			function renderChafaPreview(screenshot: ScreenshotInfo, maxWidthCells: number, previewLines: number): string[] | null {
				if (!canUseChafa()) return null;

				const width = Math.max(1, Math.floor(maxWidthCells));
				const height = Math.max(1, Math.floor(previewLines));
				const cacheKey = `${screenshot.path}:${screenshot.mtime.getTime()}:${width}x${height}:sixel`;

				if (chafaPreviewCache.has(cacheKey)) {
					return chafaPreviewCache.get(cacheKey) ?? null;
				}
				...
				try {
					...
					if (!sixel) throw new Error("empty chafa sixel output");

					// pi-tui only exempts Kitty/iTerm image lines from width checks. Add a harmless
					// Kitty APC marker so this Sixel line is treated as an image line too.
					const tuiImageLineMarker = "\x1b_Ga=d,d=I,i=0\x1b\\";
					const moveUp = height > 1 ? `\x1b[${height - 1}A` : "";
					const saveCursor = "\x1b7";
					const restoreCursor = "\x1b8";
					const preview = Array(Math.max(0, height - 1)).fill("") as string[];
					preview.push(saveCursor + moveUp + tuiImageLineMarker + sixel + restoreCursor);

					chafaPreviewCache.set(cacheKey, preview);
					return preview;
				} catch {
					chafaPreviewCache.set(cacheKey, null);
					return null;
				}
			}
			...
			function renderThumbnail(
				...
				let deleteCmd = "";
				if (terminalCapabilities.images === "kitty" && lastRenderedFrameKey && lastRenderedFrameKey !== frameKey) {
				...
				if (!thumb) {
					...
					if (!loading) {
						const chafaLines = renderChafaPreview(screenshot, maxPreviewWidthCells, previewLines);
						if (chafaLines) return chafaLines;
					}
				...
				try {
					...
					if (firstLine.includes("[Image:") || firstLine.includes("[image/")) {
						...
						// Terminal doesn't support inline images and chafa is not available - show helpful message
						const lines: string[] = [];
						...
						if (isSSH) {
							...
							lines.push(theme.fg("dim", "  Install chafa for terminal previews"));
							lines.push(theme.fg("dim", "  or use Kitty, iTerm2, WezTerm, Ghostty"));
						}
			...
			function renderZoomInspectorThumbnail(
				...
				if (terminalCapabilities.images === "kitty" && lastRenderedPath) {
					deleteCmd = deleteKittyImage(KITTY_IMAGE_ID);
				}
				lastRenderedPath = screenshot.path;
			...
			return {
				render(width: number) {
					...
					const maxPreviewWidthCells = getMaxPreviewWidthCells(safeWidth, previewZoom);
					...
					} else if (deleteWarning) {
						lines.push(" " + theme.fg("error", "\u26A0 Press d again to delete this screenshot from disk"));
						lines.push(" " + theme.fg("dim", "Any other key to cancel"));
								...
								theme.fg(
									...
									previewZoom
										? supportsKittyInspector
											? "\u2191\u2193\u2190\u2192 pan \u2022 j/k nav \u2022 +/- zoom \u2022 0 reset \u2022 z split \u2022 s/space toggle \u2022 enter done"
											: "j/k nav \u2022 +/- zoom \u2022 z split \u2022 s/space toggle \u2022 enter done"
										: "j/k nav \u2022 z zoom \u2022 s/space toggle \u2022 o open \u2022 d delete \u2022 nn nuke \u2022 enter done"
								)
						);
					} else {
						...
						lines.push(
							" " +
								theme.fg(
									...
									previewZoom
										? supportsKittyInspector
											? "\u2191\u2193\u2190\u2192 pan \u2022 j/k nav \u2022 +/- zoom \u2022 0 reset \u2022 z split \u2022 x clear all \u2022 enter done"
											: "j/k nav \u2022 +/- zoom \u2022 z split \u2022 x clear all \u2022 enter done"
										: "j/k nav \u2022 z zoom \u2022 s/space toggle \u2022 x clear all \u2022 d delete \u2022 nn nuke \u2022 enter done"
								)
						);
					}
					lines.push(border);
				...
				handleInput(data: string) {
					const screenshots = getCurrentScreenshots();
					const isNavUp = matchesKey(data, Key.up) || data === "k" || data === "K";
					const isNavDown = matchesKey(data, Key.down) || data === "j" || data === "J";
					...
					function deleteCurrentScreenshot(): void {
